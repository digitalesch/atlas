...
def test_4_eventbus_finds_correct_subscriptions(bus_and_registry):
    ...
    bus.subscribe(Subscription(Event(EventType.START_BUILD, "Start build"), lambda e: start_calls.append(e), "A"))
    bus.subscribe(Subscription(Event(EventType.END_BUILD, "Start build"), lambda e: end_calls.append(e), "B"))
...
def test_8_compiler_does_not_reference_listeners(bus_and_registry):
    ...
    compiler.compile(machine=None)

    assert len(build_events) == 1
    assert build_events[0].message == "Starting build"
...
def test_9_atlas_end_to_end_compile():
    ...
    app = Atlas()
    app.compile()  # should not raise

def test_10_subscription_matches_by_type_only_not_message(bus_and_registry):
    bus, registry = bus_and_registry
    calls = []
    bus.subscribe(Subscription(Event(EventType.LOG, "original text"), lambda e: calls.append(e), "A"))
